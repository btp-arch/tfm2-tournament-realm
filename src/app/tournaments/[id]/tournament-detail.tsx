...
export function TournamentDetail({ tournamentId }: { tournamentId: string }) {
  ...
  const [savingAction, setSavingAction] = useState<"register" | "withdraw" | "status" | null>(null);
  ...
  async function updateTournamentStatus() {
    if (!tournament || !canManageTournament || selectedStatus === tournament.status) {
      return;
    }
    ...
    try {
      ...
      setNotice("Tournament status updated.");
      await loadTournament();
      router.refresh();
    } catch (caughtError) {
