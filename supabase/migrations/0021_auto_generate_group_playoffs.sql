-- Generate group-stage playoff brackets when the final group match is finalized.

create or replace function public.get_playoff_bracket_size(player_count integer)
returns integer
language sql
immutable
...
as $$
  select case
    when player_count between 1 and 4 then 4
    when player_count between 5 and 8 then 8
    when player_count between 9 and 16 then 16
    when player_count between 17 and 32 then 32
    when player_count between 33 and 64 then 64
    else null
  end
$$;

create or replace function public.get_bracket_round_count(bracket_size integer)
returns integer
language plpgsql
immutable
set search_path = public
...
begin
  while slot_count < bracket_size loop
...
create or replace function public.get_bracket_seed_order(bracket_size integer)
returns integer[]
language plpgsql
immutable
set search_path = public
...
declare
  ...
  seed_order integer[] := array[1, 2];
  next_order integer[];
  seed integer;
  seed_index integer;
  paired_seed integer;
begin
  if bracket_size not in (2, 4, 8, 16, 32, 64) then
    raise exception 'Bracket size must be a supported power of two.';
  end if;

  while current_size < bracket_size loop
    current_size := current_size * 2;
    next_order := array[]::integer[];

    for seed_index in 1..array_length(seed_order, 1) loop
      seed := seed_order[seed_index];
      paired_seed := current_size + 1 - seed;

      if mod(seed_index - 1, 2) = 0 then
        next_order := next_order || seed || paired_seed;
      else
        next_order := next_order || paired_seed || seed;
      end if;
    end loop;

    seed_order := next_order;
  end loop;
...
create or replace function public.get_bracket_round_name(
...
as $$
declare
  round_count integer := public.get_bracket_round_count(bracket_size);
begin
  if round_number = round_count then
    return 'Final';
  end if;

  if round_number = round_count - 1 then
    return 'Semifinals';
  end if;

  if round_number = round_count - 2 then
    return 'Quarterfinals';
  end if;

  return 'Round ' || round_number::text;
end;
$$;

create or replace function public.get_tournament_round_format(
  tournament_record public.tournaments,
  bracket_size integer,
  round_number integer
)
returns public.match_format
language plpgsql
stable
set search_path = public
as $$
declare
  round_name text := public.get_bracket_round_name(bracket_size, round_number);
begin
  if round_name = 'Final' then
    return tournament_record.final_match_format;
  end if;

  if round_name = 'Semifinals' then
    return tournament_record.semifinal_match_format;
  end if;

  return tournament_record.pre_semifinal_match_format;
end;
$$;
...
create or replace function public.trigger_auto_generate_group_playoff()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if new.group_id is not null
    and new.status = 'finalized'::public.match_status
    and (
    ...
    perform public.auto_generate_group_playoff(new.tournament_id, new.finalized_by);
  end if;
...
drop trigger if exists auto_generate_group_playoff_on_match_finalized on public.matches;
create trigger auto_generate_group_playoff_on_match_finalized
  after update of status, winner_id, final_winner_score, final_loser_score on public.matches
  for each row
  execute function public.trigger_auto_generate_group_playoff();

grant execute on function public.auto_generate_group_playoff(uuid, uuid) to authenticated, service_role;
...
begin
  for tournament_record in
    ...
    group by t.id
    having bool_and(m.status = 'finalized'::public.match_status)
  loop
