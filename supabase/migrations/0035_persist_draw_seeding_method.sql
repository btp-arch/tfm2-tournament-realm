-- Persist the pre-draw seeding preference so scheduled draw automation can honor it.

alter table public.tournaments
  add column if not exists draw_seeding_method public.tournament_seeding_method not null default 'random';

alter table public.tournaments
  drop constraint if exists tournaments_draw_seeding_method_known,
  add constraint tournaments_draw_seeding_method_known
    check (draw_seeding_method in (
      'random'::public.tournament_seeding_method,
      'registration_order'::public.tournament_seeding_method,
      'check_in_order'::public.tournament_seeding_method
    ));

grant insert (draw_seeding_method) on table public.tournaments to authenticated;
grant update (draw_seeding_method, updated_at) on table public.tournaments to authenticated;
...
begin
  ...
  ordered_unseeded as (
    select
      ...
      row_number() over (
        order by
          case when manual_seed_count > 0 or tournament_record.draw_seeding_method = 'random'::public.tournament_seeding_method then random() end,
          case when manual_seed_count = 0 and tournament_record.draw_seeding_method = 'registration_order'::public.tournament_seeding_method then p.registered_at end,
          case when manual_seed_count = 0 and tournament_record.draw_seeding_method <> 'registration_order'::public.tournament_seeding_method then p.checked_in_at end,
          p.user_id
      ) as seed_rank
    from scheduled_draw_participants p
  ...
  values (
    ...
    bracket_size,
    tournament_record.draw_seeding_method,
    actor
  )
